...
const WatchListPage = ({ noOfCoinsPerPage = 10 }) => {
  ...
  //   const cachedData = localStorage.getItem('watchList');
  //   const cachedTimestamp = localStorage.getItem('watchListTimestamp');
  ...
  //     setwatchList(JSON.parse(cachedData));
  //     console.log(JSON.parse(cachedData));
  //     setFilteredwatchList(JSON.parse(cachedData));
  //     setLoading(false);
  //     return;
  //   }
  ...
  //     localStorage.setItem('watchListTimestamp', currentTime.toString()); // Save the timestamp
  //     setLoading(false);
  //   } catch (err) {
  //     setTimer(10);
  //     setError(() => {
  ...
  return (
    <>
      <div className='max-w-[1880px] min-h-[85vh] mx-auto py-6'>
        ...
        {watchList.length === 0 && (<div className='flex flex-col w-full h-[300px] justify-center items-center'>
        ...
        <div className={`${gridLayout ? 'grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))]' : 'flex flex-col'} py-6 px-8 gap-5`}>
          {watchList.slice(start * noOfCoinsPerPage - noOfCoinsPerPage, start * noOfCoinsPerPage).map((coin, index) => {
            ...
            return (
              <div
                key={coin.id}
                ref={cardRef}
                className={`${theme === 'dark' ? 'bg-[#1B1B1B]' : 'bg-gray-100'} md:py-5 md:px-6 px-2 py-2 rounded-xl flex ${gridLayout ? 'h-[300px] flex-col py-4 px-4' : 'grid  sm:grid-cols-[30%,20%,10%,40%] grid-cols-[55%,0%,15%,30%] justify-between items-center'} border-2 border-transparent transition-colors duration-300 cursor-pointer`}
                  ...
                  {gridLayout && (
                    <div className={`cursor pointer flex items-center border-2 rounded-full ${changeBorder} lg:w-[40px] lg:h-[40px] h-[25px] w-[25px] group relative`}>
                      ...
                      <div className={`flex justify-center items-center w-[25px] lg:w-[40px] rounded-full`}>
                        <FaRegTrashCan  className={`${changeColor} group-hover:text-white absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-all lg:text-md md:text-[18px] text-[12px]`} onClick={()=>removeItemFromWatchList(coin)}/>
                      </div>
                    </div>
                  )
                  }
      ...
      </div>
      {
        <div className="flex w-full items-center justify-center gap-4">
          ...
          {
            [...Array((Math.ceil(watchList.length / noOfCoinsPerPage)))].map((_, index) => (
          ...
          {start !== (Math.ceil(watchList.length / noOfCoinsPerPage)) && <div className="dots">...</div>}
          <button disabled={start >= Math.max(Math.ceil(watchList.length / noOfCoinsPerPage) - 4, 1)} onClick={() => setStart(start + 1)} className={`${start >= Math.max(Math.ceil(watchList.length / noOfCoinsPerPage) - 4, 1) ? 'disabled' : ''} md:text-4xl text-2xl`}>< FaCircleArrowRight /></button>
        </div>
      }
    </>
  );
